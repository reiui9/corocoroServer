  ...
  child = exec("curl https://coronaboard.kr/en/ > dashboard.html", function (error, stdout, stderr) {
    ...
    for (let i=0; i<obj.data.length; i++) {
      ...
      db.collection('cc').doc(element.cc.toString()).set({
        ...
        number: i+1,
        confirmed: element.confirmed?element.confirmed:0,
        death: element.death?element.death:0,
        released: element.released?element.released:0,
        candidate: element.candidate?element.candidate:0,
        negative: element.negative?element.negative:0,
        tested: element.tested?element.tested:0,
        active: element.active?element.active:0,
        confirmed_prev: element.confirmed_prev?element.confirmed_prev:0,
        death_prev: element.death_prev?element.death_prev:0,
        candidate_prev: element.candidate_prev?element.candidate_prev:0,
        negative_prev: element.negative_prev?element.negative_prev:0,
        released_prev: element.released_prev?element.released_prev:0,
        active_prev: element.active_prev?element.active_prev:0,
        population: element.population?element.population:0,
        incidence: element.incidence?element.incidence:0,
        flag: element.flag?element.flag:0
      });
